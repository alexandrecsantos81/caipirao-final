...
// import { Button } from '@/components/ui/button'; // 1. REMOVIDO
...
import { Terminal, DollarSign, Weight, ShoppingCart, BarChart2, Crown, User } from 'lucide-react';
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer } from 'recharts'; // 2. 'Rectangle' REMOVIDO da importação
...
export default function RelatoriosPage() {
  ...
  const handleTabChange = (value: string) => {
    setActiveTab(value);
    if (value === 'vendas_gerais') vendasQuery.refetch();
    if (value === 'ranking_produtos') produtosQuery.refetch();
    if (value === 'ranking_clientes') clientesQuery.refetch();
  };
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="mt-2 text-gray-600">Gere relatórios detalhados para uma visão aprofundada do seu negócio.</p>
      </div>

      <Card>
        <CardHeader>
          <CardTitle>Filtros do Relatório</CardTitle>
          <CardDescription>Selecione um intervalo de datas para gerar os relatórios abaixo.</CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <Tabs value={activeTab} onValueChange={handleTabChange} className="w-full">
        ...
        <TabsContent value="vendas_gerais">
          {vendasQuery.isLoading && <ReportLoader />}
          {vendasQuery.isError && <ReportError error={vendasQuery.error} />}
          {vendasQuery.data && !vendasQuery.isLoading && (
